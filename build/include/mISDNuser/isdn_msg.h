...
extern	msg_t		*misdnuser_msg_copy(msg_t *msg);

#define	msg_clone(m)	misdnuser_msg_copy(m)
...
static __inline__ unsigned char *msg_put(msg_t *msg, unsigned int len)
{
	...
	if(msg->tail>msg->end)
	{
		fprintf(stderr, "msg_over_panic msg(%p) data(%p) head(%p) tail(%p) len(%d)\n",
			msg, msg->data, msg->head, msg->tail, msg->len);
		return(NULL);
	}
	return tmp;
}
...
static __inline__ unsigned char * msg_pull(msg_t *msg, unsigned int len)
{
	if (len > (unsigned int)msg->len) {
		fprintf(stderr, "msg_under_panic (pull) msg(%p) data(%p) head(%p)\n",
			msg, msg->data, msg->head);
		return NULL;
        }
        pthread_mutex_lock(&msg->lock);
        unsigned char *p=(unsigned char*)__msg_pull(msg,len);
        pthread_mutex_unlock(&msg->lock);
